...
const Demo: NextPage = () => {
  ...
  const [allowSetPasswordRecovery, setAllowSetPasswordRecovery] =
    useState(true);
    ...
    try {
      ...
      if (collectResponseJSON.data?.nextAction) {
        ...
        document
          ...
          .setAttribute(
            "href",
            `https://mumbai.polygonscan.com/tx/${response.response?.transactionHash}`
          );
  ...
  return (
    <div className="login-signup-container">
      ...
      {getState === EmbeddedState.CREATING_ACCOUNT ||
      getState === EmbeddedState.READY ? (
        <>
            ...
            {allowMint ? (
              <div>
                <button
                  ...
                  disabled={getState !== EmbeddedState.READY}
                  onClick={mint}
                >
                  {"Mint NFT"}
                </button>
      ...
      {getState === EmbeddedState.MISSING_RECOVERY_METHOD ? (
        <>
          <h2 className="login-signup-title">{"Enter your recovery pin"}</h2>
